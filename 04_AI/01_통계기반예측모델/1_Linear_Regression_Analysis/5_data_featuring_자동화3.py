...
for num in range(1,len(colums_list)+1):
    ...
    for tup in combi_list:
        ...
        # 예측값의 범위가 클수록 수치를 최말단 자리까지 예측하기 어렵기 때문이다.
        ...
        for index, score in enumerate(y_predicted_rounded): # enumerate를 이용하여 index와 score를 꺼낸다
            if len(str(score)) > 1: # score의 자릿수가 1자리 이상이라면(2자리라면)
        ...
        for index in range(len(y_predicted_rounded)):
            target_price = 0
            if len(str(round(dependent_variable.values[index]))) == 1: # 만약 종속변수 값이 1자리라면
                target_price = round(dependent_variable.values[index]) # 해당 종속변수 값의 소수점 첫번째자리에서 반올림
            else:   # 만약 종속변수 값이 1자리가 아니라면
                target_price = round(dependent_variable.values[index],-1) # 해당 종속변수 값의 첫번째 자리에서 반올림
