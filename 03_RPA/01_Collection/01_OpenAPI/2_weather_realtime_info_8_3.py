...
def preprocessed_df(df):
    ...
    p_df.insert(2,'ny',[ny])
    #열이름 재정의
    #date_time	 nx  ny 강수형태 습도 시간당강수량 기온 동서바람성분 풍향 남북바람성분 풍속
    #열 순서 재정의
    #date_time	 nx  ny 기온 시간당강수량 강수형태 습도 풍속 풍향 동서바람성분 남북바람성분

    p_df.rename(columns={
        'date_time' : 'DATE_TIME', 'nx' : 'NX', 'ny' : 'NY',
         'PTY' : '강수형태', 'REH' : '습도', 'RN1' : '시간당강수량', 'T1H' : '기온', 'UUU' : '동서바람성분',
         'VEC' : '풍향', 'VVV' : '남북바람성분', 'WSD' : '풍속'
    }, inplace= True )
    redifined_columns = ['DATE_TIME', 'NX', 'NY', '기온', '시간당강수량', '강수형태', '습도', '풍속', '풍향', '동서바람성분', '남북바람성분']
    p_df = p_df[redifined_columns]
    return p_df
