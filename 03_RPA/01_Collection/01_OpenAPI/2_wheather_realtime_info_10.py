...
def preprocessed_df_to_oracle(df):
    ...
    cur = con.cursor()
    sql_insert = '''
            insert into weather(DATE_TIME, NX, NY, 기온, 시간1_강수량, 강수형태, 습도, 풍속, 풍향, 동서바람성분, 남북바람성분) 
            values(:DATE_TIME, :NX, :NY, :기온, :시간1_강수량, :강수형태, :습도, :풍속, :풍향, :동서바람성분, :남북바람성분)
            '''
    DATE_TIME = df.iloc[0]['DATE_TIME'] # 현재 데이터 프레임의 행 인덱스는 date_time이므로(피벗을 해놓은 상태) loc가 안된다.
    NX = int(df.iloc[0]['NX'])  # DB에 int 값을 넣기 위해서는 명시적으로 int 형으로 변환해줘야 한다.
    NY = int(df.iloc[0]['NY'])
    ...
    cur.execute(sql_insert,
                (   DATE_TIME, NX, NY, 기온, 시간1_강수량, 강수형태,
                    습도,  풍속, 풍향, 동서바람성분, 남북바람성분 )
                )
