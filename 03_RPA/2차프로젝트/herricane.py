...
def make_sunny_df(sheet_name) :
    input_file = 'sunny.xlsx'
    df = pd.read_excel(input_file, sheet_name=sheet_name)
    print(df)
    return df

def sunny_RoadWork_to_oracle(RoadWork) :
    current_time = time.strftime('%Y-%m-%d %H:%M:%S')
...
def sunny_ROADEVENT_to_oracle(ROADEVENT_df) :
    current_time = time.strftime('%Y-%m-%d %H:%M:%S')
...
def sunny_RoadClose_to_oracl(RoadClose_df):
    current_time = time.strftime('%Y-%m-%d %H:%M:%S')
...
def sunny_RoadCondition_to_oracle(RoadCondition_df) :
    current_time = time.strftime('%Y-%m-%d %H:%M:%S')
