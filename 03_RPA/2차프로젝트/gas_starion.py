...
def get_request_url():
    url = 'http://api.jejuits.go.kr/api/infoGasInfoList?code=860553'
...
def get_parsed_json(raw_json):
    ...
    for record in raw_json['info']:
        ...
        {"id": record.get("id"),
         ...
         "osnm": record.get("osnm"),
         "zip": record.get("zip"),
         "adr": record.get("adr"),
         "tel": record.get("tel"),
         "lpgyn": record.get("lpgyn"),
         "gisxcoor": record.get("gisxcoor"),
         "gisycoor": record.get("gisycoor")
         }
        )
    return all_data
...
def json_to_df_info(raw_json):
    all_data = []
    column_list = ["id", "poll", "osnm","zip","adr","tel","lpgyn","gisxcoor","gisycoor"]
...
file_name = f'제주도_주유소_충전소_자료.csv'
